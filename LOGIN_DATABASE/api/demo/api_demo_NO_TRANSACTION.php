...
try {
    ...
    $nomeChat = $input['nome'] ?? 'Test Chat NO TRANSACTION';
    ...
    echo json_encode([
        "step" => 2,
        "message" => "✓ Utente corrente aggiunto ($currentUser)"
    ]) . "\n";
    flush();
    ...
    foreach($partecipanti as $username) {
        ...
        echo json_encode([
            "step" => "3.$count",
            "message" => "✓ Partecipante #$count aggiunto ($username)"
        ]) . "\n";
        flush();
    ...
    echo json_encode([
        "success" => false,
        "error" => $e->getMessage(),
        "warning" => "⚠️ PROBLEMA: Chat e primi partecipanti SALVATI ma incompleti!",
        "database_state" => "INCONSISTENTE"
    ]);
}
?>